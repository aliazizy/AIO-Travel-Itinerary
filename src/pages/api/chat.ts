        ...
        const fileContents = msg.files
          .map((file) => {
            const limitedContent =
              file.content && file.content.length > settings.fileContentLimit
  ...
  try {
    ...
    if (lastMessage.files && lastMessage.files.length > 0) {
      const fileContents = lastMessage.files
        .map(
          (file) =>
            `Content from ${file.name}: ${file.content?.substring(0, 500)}...`
        )
        .join("\n");
...
export default async function handler(
  ...
  try {
    ...
    const defaultSettings: SettingsConfig = {
      ...
      systemPrompt:
        "You are AIO Travel Itinerary assistant, a helpful AI assistant specialized in travel planning and itinerary creation. Provide detailed, accurate, and helpful travel advice.",
    };
    ...
    switch (model) {
      ...
      case "ollama-llama2":
      case "ollama-mistral":
        response = await callOllama(fullMessages, model);
        break;
